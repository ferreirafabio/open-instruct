...
# Results go into multilingual_eu/ subfolder
RESULTS_SUBDIR="$OPENJURY_DIR/results/multilingual_eu"
mkdir -p "$RESULTS_SUBDIR"

# Reuse existing result directory if found
EXISTING_DIR=$(ls -d "$RESULTS_SUBDIR/${EXPERIMENT_NAME}-${BASELINE_NAME}-"*/ 2>/dev/null | head -1 || true)
if [ -n "$EXISTING_DIR" ]; then
    RESULTS_ROOT="${EXISTING_DIR%/}"
    echo "Reusing existing results directory: $RESULTS_ROOT"
else
    RESULTS_TIMESTAMP="$(date +%Y%m%d_%H%M%S)"
    RESULTS_ROOT="$RESULTS_SUBDIR/${EXPERIMENT_NAME}-${BASELINE_NAME}-${RESULTS_TIMESTAMP}"
fi
...
check_completed() {
    local dataset=$1
    for d in "$RESULTS_SUBDIR/${EXPERIMENT_NAME}-"*/; do
        [ -d "$d" ] || continue
