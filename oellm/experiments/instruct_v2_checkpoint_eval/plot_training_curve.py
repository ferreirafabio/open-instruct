...
def plot_dataset(rows: list[dict], dataset: str, output_dir: Path):
    ...
    fig.suptitle(f"Instruct SFT v2 Training Curve: {dataset}", fontsize=15, fontweight="bold")
    ...
    if len(steps_f) > 0:
        ax_win.plot(steps_f, values_f, "o-", color="#e74c3c", linewidth=1.8, markersize=6,
    ...
    if len(steps_b) > 0:
        ax_win.plot(steps_b, values_b, "s-", color="#3498db", linewidth=1.8, markersize=6,
    ...
    if has_rubric:
        ax_rub.plot(steps_r, values_r, "o-", color="#2ecc71", linewidth=1.8, markersize=6,
                    label="Trained (v2)")
