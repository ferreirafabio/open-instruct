#!/usr/bin/env bash
#SBATCH --job-name=olmo3-7b-sft-multinode
#SBATCH --partition=alldlc2_gpu-h200
#SBATCH --nodes=2
#SBATCH --gres=gpu:4
#SBATCH --ntasks-per-node=1
#SBATCH --time=1:00:00
#SBATCH --output=/work/dlclarge2/ferreira-oellm/open-instruct/oellm/sft/logs/%A_%a.%x.%N.out
#SBATCH --error=/work/dlclarge2/ferreira-oellm/open-instruct/oellm/sft/logs/%A_%a.%x.%N.err
#SBATCH --array=0-9%1

# Generic multi-node SFT training script for OLMo-core
...
#
# Usage:
#   sbatch oellm/train/train_think_sft_dolci_7b_slurm_multinode.sh
#   DATASET_PATH=data/my_data RUN_NAME=my-exp sbatch oellm/train/train_think_sft_dolci_7b_slurm_multinode.sh
...
mkdir -p "$PROJECT_ROOT/oellm/sft/logs" "$HF_DATASETS_CACHE" "$HF_MODULES_CACHE" "$HF_HUB_CACHE"
