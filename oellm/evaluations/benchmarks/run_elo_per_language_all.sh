#!/bin/bash
#SBATCH --job-name=elo-lang
#SBATCH --partition=alldlc2_gpu-h200
#SBATCH --gpus=1
#SBATCH --cpus-per-gpu=6
#SBATCH --time=04:00:00
#SBATCH --output=/work/dlclarge2/ferreira-oellm/open-instruct/oellm/evaluations/logs/elo_lang_%A_%a.log
#SBATCH --requeue

# Per-language Elo using Qwen3.5-27B judge (LMArena, single language per job)
# Array encodes: model_index * 12 + language_index
# 23 models × 12 languages = 276 jobs (array 0-275)
#
# Usage:
#   # All models, all languages
#   sbatch --array=0-275 oellm/evaluations/benchmarks/run_elo_per_language_all.sh
#
#   # Single model (e.g., model 0 = A1, all 12 languages)
#   sbatch --array=0-11 oellm/evaluations/benchmarks/run_elo_per_language_all.sh
...
# 12 EU evaluation languages (including English)
LANGUAGES_ARRAY=(en de es fr it pt pl nl cs ro el uk)

# Decode array index: model_index * 12 + lang_index
TASK_ID="${SLURM_ARRAY_TASK_ID:?Must run as array job}"
MODEL_IDX=$((TASK_ID / 12))
LANG_IDX=$((TASK_ID % 12))
...
echo "Cache key suffix: ...${N_PER_LANGUAGE}_${LANGUAGE}"
echo "=============================================="
echo ""
