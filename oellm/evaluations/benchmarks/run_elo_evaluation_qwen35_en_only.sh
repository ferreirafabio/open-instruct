#!/bin/bash
#SBATCH --job-name=elo-q35-en
#SBATCH --partition=alldlc2_gpu-h200
#SBATCH --gpus=1
#SBATCH --time=04:00:00
#SBATCH --output=/work/dlclarge2/ferreira-oellm/open-instruct/oellm/evaluations/logs/elo_q35_en_%A_%a.log
#SBATCH --requeue
# no node exclusions

# English-only Elo Rating estimation using Qwen3.5-27B judge (LMArena)
# Runs 200 English battles only (--languages en) for each model.
# Cache key includes "en" suffix → no collision with multilingual runs.
#
# Usage:
#   # Single model
#   sbatch oellm/evaluations/benchmarks/run_elo_evaluation_qwen35_en_only.sh eu-D1-90en
#
#   # All 13 models (12 EU + baseline) as array job
#   sbatch --array=0-12 oellm/evaluations/benchmarks/run_elo_evaluation_qwen35_en_only.sh
...
# Array job model mapping (0-12): 12 EU models + baseline
MODELS_ARRAY=(
    "eu-A1-90en-step238"
    "eu-A2-80en-step228"
    "eu-A3-70en-step216"
...
if [ -n "${SLURM_ARRAY_TASK_ID:-}" ]; then
    ...
    MODEL_SYMLINK="${1:?Usage: sbatch run_elo_evaluation_qwen35_en_only.sh <model-symlink>}"
fi

N_PER_LANGUAGE=200  # 200 English battles
...
echo ""
echo "=============================================="
echo "Elo RATING ESTIMATION — ENGLISH ONLY (Qwen3.5-27B judge)"
echo "=============================================="
...
echo "Languages:        en (English only)"
echo "Swap Mode:        $SWAP_MODE"
echo "Cache key suffix: ...${N_PER_LANGUAGE}_en"
echo "=============================================="
echo ""
...
# === Run Elo evaluation (English only) ===
cd "$OPENJURY_DIR"
...
echo ""
echo "=============================================="
echo "Elo Estimation Complete (English only): $MODEL_SYMLINK (Qwen3.5-27B judge)"
echo "=============================================="
