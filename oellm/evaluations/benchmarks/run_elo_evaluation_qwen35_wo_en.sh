#!/bin/bash
#SBATCH --job-name=elo-q35-noen
#SBATCH --partition=alldlc2_gpu-h200
#SBATCH --gpus=1
#SBATCH --time=24:00:00
#SBATCH --output=/work/dlclarge2/ferreira-oellm/open-instruct/oellm/evaluations/logs/elo_q35_noen_%A_%a.log
#SBATCH --requeue
# no node exclusions

# ELO Rating estimation using Qwen3.5-27B judge (LMArena, WITHOUT English)
# Judge runs as vllm serve background process; evaluated model uses VLLM in-process.
# Balanced: 200 battles/language × 11 EU languages = ~2,200 battles per model
#
# Usage:
#   # Single model
#   sbatch oellm/evaluations/benchmarks/run_elo_evaluation_qwen35_wo_en.sh eu-D1-90en
#
#   # All 12 EU models as array job
#   sbatch --array=0-11 oellm/evaluations/benchmarks/run_elo_evaluation_qwen35_wo_en.sh
...
if [ -n "${SLURM_ARRAY_TASK_ID:-}" ]; then
    ...
    MODEL_SYMLINK="${1:?Usage: sbatch run_elo_evaluation_qwen35_wo_en.sh <model-symlink>}"
fi

N_PER_LANGUAGE=200  # Balanced sampling: 200 battles per language
...
# EU languages WITHOUT English
EU_LANGUAGES_NO_EN="de es fr it pt pl nl cs ro el uk"
...
echo ""
echo "=============================================="
echo "ELO RATING ESTIMATION w/o English (Qwen3.5-27B judge)"
echo "=============================================="
...
echo "Languages:        $EU_LANGUAGES_NO_EN (NO ENGLISH)"
...
$VENV_PYTHON judgearena/estimate_elo_ratings.py \
    ...
    \
    --languages $EU_LANGUAGES_NO_EN \
    --n_instructions_per_language $N_PER_LANGUAGE

echo ""
echo "=============================================="
echo "ELO Estimation (w/o en) Complete: $MODEL_SYMLINK (Qwen3.5-27B judge)"
echo "=============================================="
